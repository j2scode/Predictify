...
The following summarizes the analyses and outlines the structure of the pilot corpus to be used as a basis for building the training, validation and test sets.  `r figr("pilotAnalysis-M", TRUE, type="Table")` summarizes the results of the sample size analysis.

`r figr("pilotAnalysis-M", TRUE, type="Table")`: Sample size. Sample sizes are in tokens
...
The lexical diversity-based estimate of `r pilotDesign$comparison[4,3]` tokens was significantly greater than that of the lexical feature analysis. Taking a conservative approach, the lexical diversity-based estimate was selected for the pilot corpus. The somewhat surprising result was that lexical diversity and density representation could be achieved with just `r pilotDesign$comparison[4,7]`% of the HC Corpus. Also notable was the vast difference in the proportional representation of the registers.  Given the lexical diversity of the news register, `r pilotDesign$comparison[2,7]`% of the news register was required to represent linguistic characteristics of the population of news content in the HC Corpus, versus `r pilotDesign$comparison[1,7]`% for the blog and twitter registers. Despite this, the news register accounted for just `r pilotDesign$comparison[2,8]`% of the pilot corpus. The blogs and twitter registers accounted for Some `r sum(pilotDesign$comparison[1,8], pilotDesign$comparison[3,8])`% of the pilot.

To work with round numbers, the pilot corpus sample size was extrapolated out to 5%, while maintaining the proportional representation of each register. The following table outlines the extrapolated pilot corpus sample size assuming a `r samplingUnit[[length(samplingUnit)]]$size` token sampling unit as given by the lexical feature analysis.
...
The sample size raw estimates were `r round(pilotDesign$pilot[4,4],0)` tokens and `r pilotDesign$pilot[4,5]` sentences. Using the registers mean sentence length, the number of sentences per 2000 word chunk was calculated. The final sample size estimates were given as a multiple of chunk size.
...
`r figr("vocabularyVerification-M", TRUE, type="Table")`: Training corpus vocabulary OOV Rates
...
This analysis illustrates a feature of a "zipfian" distribution. Despite a vocabulary OOV rate of `r round(coverage[4,4] / coverage[4,2] * 100, 0)`% the pilot corpus covered `r round(coverage[4,8], 0)`% of the tokens in the HC Corpus. Indeed, a total, `r round(coverage[4,5] / coverage[4,3] * 100,0)`% of the corpus provided `r round(coverage[4,8], 0)` coverage.
...
### Model Corpora Design
The model corpora included the training, validation, and test corpora. The validation and test sets would be allocated on 5% pilot corpus each. As `r figr("corporaDesign-M", TRUE, type="Table")`, the language model will be evaluated on training sets of 10%, 20%, 35% and 50% of the HC Corpus.

`r figr("corporaDesign-M", TRUE, type="Table")`: The model corpora design. Training sets are alpha thru delta.
```{r corporaDesign-M, echo=F, message=FALSE,  eval=T, results='asis'}
kable(pilotDesign$corporaDesign, digits = 0, format.args = list(big.mark = ','))
```

The next section summarizes the **exploratory data analysis** of the **delta** training set.
...
`r figr("trainingSetStats-M", TRUE, type="Table")`: Training Set Descriptive Statistics
```{r trainingSetStats-M, echo=F, message=FALSE,  eval=T, results='asis'}
training <- plotCorpusSummary(trainingDataAnalysis)
kable(training, digits = 0, format.args = list(big.mark = ','))
```

Reducing the size of the corpus by `r round((cleanDataAnalysis$featureMatrix$tokens[4] - trainingDataAnalysis$featureMatrix$tokens[4]) / cleanDataAnalysis$featureMatrix$tokens[4] * 100, 0)`% while retaining the lexical diversity and feature distribution of the HC Corpus, was the **key finding** of this analysis.
...
```
`r figr("diversityPlots-M", TRUE, type="Figure")`: Training Corpus Lexical Diversity Analysis
...
`r figr("densityAnalysis-M", TRUE, type="Table")`: Lexical Density Analysis of Training Corpus
...
```{r loadCleanTextNGrams-M, echo=F, message=FALSE, cache=T, results='hide'}
nGrams <- loadNGrams(corpora$training$clean$nGrams$text)
```
...
zPlot <- plotZipf(nGramFreqAnalysis[[1]]$tfl, register = 'Training Corpus Unigrams', color = 'orangered3')
unigramZipfPlot <- zPlot$zipf
zPlot <- plotZipf(nGramFreqAnalysis[[2]]$tfl, register = 'Training Corpus Bigrams', color = 'navyblue')
bigramZipfPlot <- zPlot$zipf
zPlot <- plotZipf(nGramFreqAnalysis[[3]]$tfl, register = 'Training Corpus Trigrams', color = 'springgreen4')
trigramZipfPlot <- zPlot$zipf
zPlot <- plotZipf(nGramFreqAnalysis[[3]]$tfl, register = 'Training Corpus quadgrams', color = 'firebrick4')
...
coveragePlot <- plotNGramCoverage(nGramCoverage[[1]], type = 'Training Corpus Unigrams', color = 'orangered3')
unigramCoveragePlot <- coveragePlot
coveragePlot <- plotNGramCoverage(nGramCoverage[[2]], type = 'Training Corpus Bigrams', color = 'navyblue')
bigramCoveragePlot <- coveragePlot
coveragePlot <- plotNGramCoverage(nGramCoverage[[3]], type = 'Training Corpus Trigrams', color = 'springgreen4')
trigramCoveragePlot <- coveragePlot
coveragePlot <- plotNGramCoverage(nGramCoverage[[4]], type = 'Training Corpus quadgrams', color = 'firebrick4')
...
The training corpus comprised some `r nGrams[[1]]@Dim[2]` unigrams, `r nGrams[[2]]@Dim[2]` bigrams, `r nGrams[[3]]@Dim[2]` trigrams, and `r nGrams[[4]]@Dim[2]` quadgrams. The following illuminates the top 50 features by frequency for each N-Gram order.
...
```{r wordUnigramCloud-M, echo=F, fig.width=3.6, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-2), out.extra='style="float:left"'}
...
```{r wordBigramCloud-M, echo=F, fig.width=3.6, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-4), out.extra='style="float:left"'}
...
```{r wordTrigramCloud-M, echo=F, fig.width=3.6, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-6), out.extra='style="float:left"'}
...
```{r plotQuadgrams-M, echo=F, message=FALSE, results='hide', cache=T, dependson=c(-7), out.extra='style="float:left"'}
...
### Syntactic (POS) N-Gram Analysis
```{r loadCleanPOSNGrams-M, echo=F, eval=T, message=F, warning=F, cache=T}
nGrams <- loadNGrams(corpora$training$clean$nGrams$pos)
```
The training corpus comprised some `r nGrams[[1]]@Dim[2]` POS unigrams, `r nGrams[[2]]@Dim[2]` POS bigrams, and `r nGrams[[3]]@Dim[2]` POS trigrams. The following reveals the top features by frequency for each Ngram.

```{r plotPOSUnigrams-M, echo=F, fig.width=5.8, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-1), ,out.extra='style="float:left"'}
nGramPlotData <- plotNgrams(nGrams[[1]], type = "Unigrams", color = "steelblue", features = 40)
nGramPlotData$boxPlot
```

```{r posUnigramCloud-M, echo=F, fig.width=3.6, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-2), out.extra='style="float:left"'}
textplot_wordcloud(nGrams[[1]], random.order = FALSE, rot.per = .25,
                   colors = RColorBrewer::brewer.pal(8,"Dark2"))
```
`r figr("plotPOSUnigrams-M", TRUE, type="Figure")`: 40 Most frequent of $N$ = `r nGrams[[1]]@Dim[2]` unigrams

```{r plotPOSBigrams-M, echo=F, fig.width=5.8, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-3), out.extra='style="float:left"'}
nGramPlotData <- plotNgrams(nGrams[[2]], type = "Bigrams", color = "orange", features = 50)
nGramPlotData$boxPlot
```

```{r posBigramCloud-M, echo=F, fig.width=3.6, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-4), out.extra='style="float:left"'}
textplot_wordcloud(nGrams[[2]], random.order = FALSE, rot.per = .25,
                   colors = RColorBrewer::brewer.pal(8,"Dark2"))
```
`r figr("plotPOSBigrams-M", TRUE, type="Figure")`: 50 Most frequent of $N$ = `r nGrams[[2]]@Dim[2]` bigrams

```{r plotPOSTrigrams-M, echo=F, fig.width=5.8, fig.height=3, message=FALSE, results='hide', cache=T, dependson=c(-5), out.extra='style="float:left"'}
nGramPlotData <- plotNgrams(nGrams[[3]], type = "Trigrams", color = "springgreen4", features = 50)
nGramPlotData$boxPlot
```

```{r posTrigramCloud-M, echo=F, fig.width=3.6, fig.height=3, message=FALSE, results='hide', cache=T,  dependson=c(-6), out.extra='style="float:left"'}
textplot_wordcloud(nGrams[[3]], random.order = FALSE, rot.per = .25,
                   colors = RColorBrewer::brewer.pal(8,"Dark2"))
```
`r figr("plotPOSTrigrams-M", TRUE, type="Figure")`: 50 Most frequent of $N$ = `r nGrams[[3]]@Dim[2]` trigrams
