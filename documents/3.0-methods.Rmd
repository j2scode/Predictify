...
Therefore, the aim at this stage was to identify a minimum size **pilot corpus** that was representative of the HC Corpus according to the above definition. Since the language model will be evaluated on training sets of various sizes, the training sets would be comprised of 2-10 pilot corpora and the validation and training sets would constitute a two pilot corpora each.

Two analyses were undertaken to address the question of representativeness.  The lexical diversity analysis applied Zipf's law and word frequency distribution analysis to the problem of language coverage and lexical size.  The lexical feature analysis examined the distribution of lexical features to ascertain optimal sampling unit, register, and overall corpus size. 
...
The model corpora, consisting of four training sets, a validation set and a test set would be comprised of one or more pilot corpora.  Training sets alpha, beta, gamma, and delta would contain two, four, seven, and ten pilot corpora respectively.  This amounted to about 10%, 20%, 35%, and 50% of the HC Corpus. The validation and test sets would be comprised of a two pilot corpora each, accounting for about 10% of the HC Corpus, respectively. The design is summarized below.
...
zPlot <- plotZipf(nGramFreqAnalysis[[3]]$tfl, register = 'Pilot Corpus quadgrams', color = 'firebrick4')
...
## Model Corpora 
The model corpora included the training, validation, and test corpora. The validation and test sets would be allocated on 5% training corpus each. As `r figr("corporaDesign-M", TRUE, type="Table")`, the language model will be evaluated on training sets of 10%, 20%, 35% and 50% of the HC Corpus.

`r figr("corporaDesign-M", TRUE, type="Table")`: The model corpora design. training sets are alpha thru delta.
```{r corporaDesign-M, echo=F, message=FALSE,  eval=T, results='asis'}
kable(trainingDesign$corporaDesign, digits = 0, format.args = list(big.mark = ','))
```
