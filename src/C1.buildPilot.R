...
#' @param registers - the corpus register meta data 
#' @author John James
#' @export
buildPilot <- function(clean, pilot, design, registers) {
  ...
  lapply(seq_along(korpus), function(d) {
    ...
    # Save the file
    pilotData <- list()
    pilotData$directory <- file.path(pilot$directory, 'documents')
    pilotData$fileName <- registers[[d]]$fileName
    pilotData$data <- chunks
    saveFile(pilotData)
  })
