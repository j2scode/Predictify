...
mknAbsCount <- function(mkn, nGrams) {
  ...
  summaryCounts <- rbindlist(lapply(seq_along(nGrams), function(x) {
    ...
    # Summarize counts 
    s <- list()
    s$nGram <- mkn$args$counts[[x]]$fileDesc
    s$Count <- nfeature(nGram)
    s
  }))
  
  # Save Summary counts
  mkn$args$summary$data <- summaryCounts
  saveObject(mkn$args$summary)

  # Log Results
  logR(paste(mkn$args$mName, 'counts'), startTime, 
       mkn$args$counts[[1]]$directory, 'various')
